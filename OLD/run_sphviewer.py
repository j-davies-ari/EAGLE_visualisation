...
phi_list = np.arange(0.,359.,1) # Default is to make a 360 degree rotating set of images for a movie
if 'single' in argv:
    phi_list = [0.,]
...
try:
    ...
    print 'Please enter command-line inputs for "simulation", "run","redshift","group number", "property" (stars/gasmass/xrays/temperature)'
    exit()

tag = snapdict[redshift]
...
save_loc = '/home/arijdav1/Dropbox/phd/figures/sph_movies/'+sim+'/'+run+'/'+tag+'/group'+str(gn)+'/'+prop+'/'
...
try:
    data = h5.File('/data5/arijdav1/saved_regions/'+sim+'_'+run+'/'+tag+'/'+halotype+'/group'+str(gn)+'.hdf5','r')
except IOError:
...
if not mean_mass_weighted:
    ...
    for p, phi in tqdm(enumerate(phi_list)):
        ...
        #im1 = plt.imshow(img, extent=extent, origin='lower', cmap=cmap)
        ...
        if 'single' in argv:
            ...
            plt.savefig(save_loc+'group'+str(gn)+'_phi%03d'%(p)+'.png')
        plt.close()
