...
one function to project them all
and to the output bind them
...
# eos         for particles not on the equation of state (boolean)
#
...
def getBenOpp1chemabundtables(vardict,excludeSFR,eltab,hab,ion,last=True,updatesel=True,misc=None):
    # ion names used here and in tale naming -> ions in ChemicalAbundances table
    ...
    vardict.readif('ChemicalAbundances/%s'%(iontranslation[ion]),region = 'auto',rawunits = False,out =False, setsel = None,setval = None)
    if ion != 'h1':
        ...
        vardict.particle['ChemicalAbundances/%s'%(iontranslation[ion])][vardict.particle[eltab]==0] = 0. #handle /0 erros properly: no elements -> no ions of that element 
        vardict.add_part('ionfrac',vardict.particle['ChemicalAbundances/%s'%(iontranslation[ion])])
    else:
        vardict.add_part('ionfrac',vardict.particle['ChemicalAbundances/%s'%(iontranslation[ion])])
    vardict.delif('ChemicalAbundances/%s'%(iontranslation[ion]),last=last)
...
def nameoutput(ptypeW,simnum,snapnum,version,kernel,npix_x,L_x,L_y,L_z,centre,BoxSize,hconst,excludeSFRW,excludeSFRQ,velcut,axis,var,abundsW,ionW,parttype,ptypeQ,abundsQ,ionQ,quantityW,quantityQ,simulation,LsinMpc,misc): 
    ...
    if ptypeQ is None: #output outputW name
        if ptypeW == 'coldens' or ptypeW == 'emission':
            resfile = ol.npzdir + '%s_%s_%s_%s_test%s_%s_%sSm_%spix_%sslice' %(ptypeW,ionW,ssimnum,snapnum,str(version),sabundsW,kernel,str(npix_x),sLp) + zcen + xypos + axind + SFRindW + vind

        elif ptypeW == 'basic':
            resfile = ol.npzdir + '%s%s_%s_%s_test%s_%sSm_%spix_%sslice' %(squantityW,sparttype,ssimnum,snapnum,str(version),kernel,str(npix_x),sLp) + zcen + xypos + axind + SFRindW + vind
    ...
    if ptypeQ is not None: # naming for quantityQ output
        ...
        resfile = ol.npzdir + '%s_%s%s_%s_%s_test%s_%sSm_%spix_%sslice' %(squantityQ,squantityW,sparttype,ssimnum,snapnum,str(version),kernel,str(npix_x),sLp) + zcen + xypos + axind + vind

    
    if misc is not None: 
        # key-value expansion means names are not strictly deterministic for misc options, but the naming should be unamiguoug, so that's ok
...
def make_map(simnum, snapnum, centre, L_x, L_y, L_z, npix_x, npix_y, \
    ...
    'usechemabundtables': 'BenOpp1'
                           use Ben Oppenheimer's ChemicalAbundances tables in 
                           e.g. 
