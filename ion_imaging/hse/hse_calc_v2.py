...
angmom_indexes = rg.distanceindexes(coords_stars,gal_coords_pMpc,0.03/hubbleparam,boxsize_pMpc)
...
phi, theta, psi = rotg.calc_angmom_axisangles(gal_coords_pMpc, gal_vels_kms, coords_stars[angmom_indexes], vels_stars[angmom_indexes], boxsize_pMpc)
...
gas_indexes = rg.distanceindexes(coords_gas,gal_coords_pMpc,Rcut,boxsize_pMpc)
print "Finished gas indexing, gas_indexes length = ", len(coords_gas[gas_indexes]) 
DM_indexes = rg.distanceindexes(coords_DM,gal_coords_pMpc,Rcut,boxsize_pMpc)
print "Finished DM indexing, DM_indexes length = ", len(coords_DM[DM_indexes]) 
stars_indexes = rg.distanceindexes(coords_stars,gal_coords_pMpc,Rcut,boxsize_pMpc)
print "Finished stellar indexing, star_indexes length = ", len(coords_stars[stars_indexes]) 
BH_indexes = rg.distanceindexes(coords_BH,gal_coords_pMpc,Rcut,boxsize_pMpc)
...
#angmom_indexes = rg.distanceindexes(coords_cool_rot,gal_coords_pMpc,0.10/hubbleparam,boxsize_pMpc)
# This was a test to see if I rotated the galaxy correctly.  I'm taking it out.  
#print "L489: phi, theta, psi = ", phi, theta, psi
#rotg.calc_angmom_axisangles(gal_coords_pMpc, gal_vels_kms, coords_cool_rot[angmom_indexes], vels_cool_rot[angmom_indexes], boxsize_pMpc)
...
angmom_indexes = rg.distanceindexes(coords_stars_rot,gal_coords_pMpc,0.03/hubbleparam,boxsize_pMpc)
...
if (chem): # individual ions- don't use.  
    ...
    wg.chem("snap_%s.0.hdf5"%galidstr, h1, c2, c3, c4, n5, o6, o7, o8, ne8, mg2, si2, si3, si4)
...
if (M200>13.5): # if a higher mass halo- use fewer radial bins (for my 2018 paper)
    nradial= 60 
...
if dofigs:
    for k in xrange(3):
        ...
        sigsq_gas_dir = (vels_gas_rot[:,k] - gal_vels_kms[k])**2.
