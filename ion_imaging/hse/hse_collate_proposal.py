...
fig_vtanhot_coll = plt.figure(figsize=(4.25,4.25))
...
#            lvtanhot = ax_vtanhot_coll.plot(10**lRbins, vtan_hot_50[h], color='black', lw=4,alpha=1.0,ls='-',zorder=20-h)
            lvtanhot = ax_vtanhot_coll.plot(10**lRbins, vtan_hot_50[h], color="orangered", lw=4,alpha=1.0,ls='-',zorder=20-h,label=labelwrite)
            #if(pversion==0): ax_vtanhot_coll.fill_between(10**lRbins,vtan_hot_25[h],vtan_hot_75[h],facecolor=colorbins[h],alpha=0.2)
            if((h==2) & (pversion==0) & (plotallhalobins==0)):lvtancoolLstar = ax_vtanhot_coll.plot(10**lRbins, vtan_cool_50[h], color="limegreen", lw=4,alpha=1.0,ls='-',zorder=20-h)

#            lsigtanhot = ax_vtanhot_coll.plot(10**lRbins, sigtan_hot_50[h], color='black', lw=4,alpha=1.0,ls=':',zorder=20-h)
            lsigtanhot = ax_vtanhot_coll.plot(10**lRbins, sigtan_hot_50[h], color="orangered", lw=4,alpha=1.0,ls=':',zorder=20-h)
            #if(pversion==0): ax_vtanhot_coll.fill_between(10**lRbins,sigtan_hot_25[h],sigtan_hot_75[h],facecolor=colorbins[h],alpha=0.2)
            if((h==2) & (pversion==0) & (plotallhalobins==0)):lsigtancoolLstar = ax_vtanhot_coll.plot(10**lRbins, sigtan_cool_50[h], color="limegreen", lw=4,alpha=1.0,ls=':',zorder=20-h)
...
ax_vtanhot_coll.set_ylim(0,1.5)
ax_vtanhot_coll.set_xticks([0.03,0.1,0.3,1.0,2.0])
ax_vtanhot_coll.get_xaxis().set_major_formatter(mpl.ticker.ScalarFormatter())
ax_vtanhot_coll.set_yticks([0.0,0.5,1.0,1.5])
ax_vtanhot_coll.set_xlabel('$R/R_\mathrm{vir}$',fontsize=16)
ax_vtanhot_coll.set_ylabel('$v/v_\mathrm{vir}$',fontsize=16)
ax_vtanhot_coll.plot([1,1],[-10,10],color='k',lw=1,ls=':')
ax_vtanhot_coll.plot([-10,10],[1,1],color='k',lw=1,ls=':')
...
if('lvtancoolLstar' in vars()):
    second_legend = ax_vtanhot_coll.legend((lvtancoolLstar[0],lsigtancoolLstar[0],lvtanhot[0],lsigtanhot[0]),(r'$v_{\mathrm{tan,cool}}$',r'$\sigma_{\mathrm{tan,cool}}$',r'$v_{\mathrm{tan,hot}}$',r'$\sigma_{\mathrm{tan,hot}}$'),loc="upper right", ncol=1,fontsize=14)
...
fig_vtanhot_coll.subplots_adjust(left=0.20, bottom=0.20,top=0.95,right=0.95)
fig_vtanhot_coll.savefig('Vtan_cool_hot_coll.' + lsfile + '.png')
