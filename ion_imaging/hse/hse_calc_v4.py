...
#sys.path.append('/cosma/home/dp004/dc-oppe1/pythonCode/coldens_ben/')
import coldens_ben.coldens as coldens
#sys.path.append('/cosma/home/dp004/dc-oppe1/pythonCode/nastasha_ionbal/')
...
def write_radial_column(filename,arr,ngrid,lgrid,lin,Nthresh1,Nthresh2,Nthresh3):
    ...
    arr1 = np.where(np.log10(arr)>=Nthresh1, 1.0, 0.0)
    arr2 = np.where(np.log10(arr)>=Nthresh2, 1.0, 0.0)
    arr3 = np.where(np.log10(arr)>=Nthresh3, 1.0, 0.0)

    arr1_hist = np.histogram(dist,nbins,range=[1e-05,lgrid*1e+03/2.],weights=arr1)[0]
    arr2_hist = np.histogram(dist,nbins,range=[1e-05,lgrid*1e+03/2.],weights=arr2)[0]
    arr3_hist = np.histogram(dist,nbins,range=[1e-05,lgrid*1e+03/2.],weights=arr3)[0]

    
    f = file(filename, 'w')
    f.write('#kpclo kpchi col cov%4.1f cov%4.1f cov%4.1f n\n'%(Nthresh1,Nthresh2,Nthresh3))

    for i in xrange(nbins):
        if lin > 0:
            f.write('%5.1f %5.1f %5.2f %5.3f %5.3f %5.3f %5d\n'%(bins[i],bins[i+1],np.log10(hist[i]/npart[i]),arr1_hist[i]/npart[i],arr2_hist[i]/npart[i],arr3_hist[i]/npart[i],npart[i]))
        else:
...
write_hdf5 = 1
metals = 1
chem = 1 # tag to do chemical reading in (for column density maps)
equ_chem = 1 # tag to assume equilibrium look-up tables (via Nastasha).
dofigs = 1 # tag to make column density map figures.  
vel_figs = 0
...
do_hse_calcs = 0 # Do Hydrostatic equilibrium cacluatlions, set to no.  
...
if (runlabel=='halo'): # Parsing of galidstr  
    ...
    Mstar = float(haloname.split("_")[3])
    SFRcentral = float(haloname.split("_")[4])
    haloinfostr = 'lg M$_{200}=%4.1f$, lg M$_{*}=%3.1f$, SFR$=%4.2f$'%(M200,Mstar,SFRcentral)
...
print "snap_intag= ", snap_intag, M200, Mstar, SFRcentral, redshift
#snapnum = int(snap_intag.split("_")[0])
snapnum = int(galidstr.split("_z")[0].split("_")[-1])

if(snapnum > 100): #Need a better way to identify snipshot
    sniptag = "SNIP"
    hdf5filename0 = sim + "/snipshot_" + snap_intag + "/snip_" + snap_intag + ".0.hdf5"
...
omegaM = 0.307
omegaL = 1-omegaM
omegaratio = (omegaM+omegaL/(1+redshift)**3) # proportional to the Hubble expansion at the given redshift. 
...
if(hashtable):
    ...
    print "length_x = ", len(coords_DM), min(coords_DM[:,0]), max(coords_DM[:,0])
    print "length_y = ", len(coords_DM), min(coords_DM[:,1]), max(coords_DM[:,1])
    print "length_z = ", len(coords_DM), min(coords_DM[:,2]), max(coords_DM[:,2])
...
if(hashtable):
    ...
    ID_stars = rg.PartIDs(sniptag,sim,snap_intag,4,region=[xlow,xhi,ylow,yhi,zlow,zhi])
    ...
    ID_stars = rg.PartIDs(sniptag,sim,snap_intag,4)
...
if(hashtable):    
    ...
    ID_gas = rg.PartIDs(sniptag,sim,snap_intag,0,region=[xlow,xhi,ylow,yhi,zlow,zhi])

else:
    mass_gas = rg.mass(sniptag,sim,snap_intag,0)
    ID_gas = rg.PartIDs(sniptag,sim,snap_intag,0)
...
#coords_gas_rot = coords_gas[gas_indexes]
#coords_stars_rot = coords_stars[stars_indexes]
#coords_DM_rot = coords_DM[DM_indexes]

print "Rotate galaxy_x= ", min(coords_gas_rot[:,0]), max(coords_gas_rot[:,0])
print "Rotate galaxy_y= ", min(coords_gas_rot[:,1]), max(coords_gas_rot[:,1])
print "Rotate galaxy_z= ", min(coords_gas_rot[:,2]), max(coords_gas_rot[:,2])
...
if (cosmoowls): # hopefully code that you shouldn't need to worry about.  
    ...
    carbon = carbon[gas_indexes]
    oxygen = oxygen[gas_indexes]
    oxy = oxygen/(hydrogen*16.0)
    car = carbon/(hydrogen*12.0)
...
if (chem): # individual ions- don't use.  
    ...
    if (equ_chem):
        h1_frac = ionbal.find_ionbal(redshift,"h1",np.log10(nH[gas_indexes]), np.log10(T[gas_indexes]))
        c4_frac = ionbal.find_ionbal(redshift,"c4",np.log10(nH[gas_indexes]), np.log10(T[gas_indexes]))
        o6_frac = ionbal.find_ionbal(redshift,"o6",np.log10(nH[gas_indexes]), np.log10(T[gas_indexes])) 
        o7_frac = ionbal.find_ionbal(redshift,"o7",np.log10(nH[gas_indexes]), np.log10(T[gas_indexes]))
        o8_frac = ionbal.find_ionbal(redshift,"o8",np.log10(nH[gas_indexes]), np.log10(T[gas_indexes]))
    else:
        h1,c2,c3,c4,n5,o6,o7,o8,ne8,mg2,si2,si3,si4 = rg.chemistryprimary(sniptag, sim, snap_intag)
        h1 = np.where((np.isinf(h1[gas_indexes])) | (np.isnan(h1[gas_indexes])), 0.0, h1[gas_indexes])
        c2 = np.where((np.isinf(c2[gas_indexes])) | (np.isnan(c2[gas_indexes])), 0.0, c2[gas_indexes])
        c3 = np.where((np.isinf(c3[gas_indexes])) | (np.isnan(c3[gas_indexes])), 0.0, c3[gas_indexes])
        c4 = np.where((np.isinf(c4[gas_indexes])) | (np.isnan(c4[gas_indexes])), 0.0, c4[gas_indexes])
        n5 = np.where((np.isinf(n5[gas_indexes])) | (np.isnan(n5[gas_indexes])), 0.0, n5[gas_indexes])
        o6 = np.where((np.isinf(o6[gas_indexes])) | (np.isnan(o6[gas_indexes])), 0.0, o6[gas_indexes])
        o7 = np.where((np.isinf(o7[gas_indexes])) | (np.isnan(o7[gas_indexes])), 0.0, o7[gas_indexes])
        o8 = np.where((np.isinf(o8[gas_indexes])) | (np.isnan(o8[gas_indexes])), 0.0, o8[gas_indexes])
        ne8 = np.where((np.isinf(ne8[gas_indexes])) | (np.isnan(ne8[gas_indexes])), 0.0, ne8[gas_indexes])
        mg2 = np.where((np.isinf(mg2[gas_indexes])) | (np.isnan(mg2[gas_indexes])), 0.0, mg2[gas_indexes])
        si2 = np.where((np.isinf(si2[gas_indexes])) | (np.isnan(si2[gas_indexes])), 0.0, si2[gas_indexes])
        si3 = np.where((np.isinf(si3[gas_indexes])) | (np.isnan(si3[gas_indexes])), 0.0, si3[gas_indexes])
        si4 = np.where((np.isinf(si4[gas_indexes])) | (np.isnan(si4[gas_indexes])), 0.0, si4[gas_indexes])
        
        h1_frac = h1
        c4_frac = c4/(carbon/(12.0*hydrogen))
        o6_frac = o6/(oxygen/(16.0*hydrogen))
        o7_frac = o7/(oxygen/(16.0*hydrogen))
        o8_frac = o8/(oxygen/(16.0*hydrogen))
...
if(write_hdf5):
    wg.halogalparams("snap_%s.0.hdf5"%galidstr, M200, Mstar, SFRcentral, R200, v200)
    wg.PartIDs("snap_%s.0.hdf5"%galidstr,ID_gas[gas_indexes],0)
    wg.PartIDs("snap_%s.0.hdf5"%galidstr,ID_stars[stars_indexes],4)
...
do_ion_abel_transform = 0 # I experimented around with this, but it went nowhere and is wrong.
if(do_ion_abel_transform):
    if(equ_chem):
        fions = open("ions_%s.dat"%galidstr,"w")

        N_HI = br.abel_transform(mass_gas*hydrogen*h1_frac,R_gas,0.0,0.5,50,1.0)[0]
        N_OVI = br.abel_transform(mass_gas*oxygen*o6_frac,R_gas,0.0,0.5,50,16.0)[0]
        N_OVII = br.abel_transform(mass_gas*oxygen*o7_frac,R_gas,0.0,0.5,50,16.0)[0]
        N_OVIII = br.abel_transform(mass_gas*oxygen*o8_frac,R_gas,0.0,0.5,50,16.0)[0]
        N_CIV, b_bins = br.abel_transform(mass_gas*carbon*c4_frac,R_gas,0.0,0.5,50,m_carbon)
        fions.write("#bMpc N_HI N_CIV N_OVI N_OVII N_OVIII\n")
        for i in range(len(b_bins)):
            fions.write("%5.3f %5.2f %5.2f %5.2f %5.2f %5.2f\n"%(b_bins[i],N_HI[i],N_CIV[i],N_OVI[i],N_OVII[i],N_OVIII[i]))

        fions.close()

        h1 = h1_frac
        c4 = c4_frac*(carbon/(12.0*hydrogen))
        o6 = o6_frac*(oxygen/(16.0*hydrogen))
        o7 = o7_frac*(oxygen/(16.0*hydrogen))
        o8 = o8_frac*(oxygen/(16.0*hydrogen))
        #car = carbon/(12.0*hydrogen)
        #oxy = oxygen/(16.0*hydrogen)
        

# indexes where I select different gas- noISM is no star-forming (i.e. only CGM) gas, and nosatISM is more obscure, but I wanted to calculate gas masses using these different categories below.  
...
print "MASSES= ", np.log10(M_DM), np.log10(M_CGM), " ISMs= ", np.log10(M_ISM), np.log10(M_ISM_30kpc), " Stars= ", np.log10(M_stars),Mstar,np.log10(M_stars_30kpc)
...
# Writing one line file of masses in the halo.  
fmasses = open("masses_%s.dat"%galidstr,"w")
fmasses.write("galidstr= %5.2f %5.2f %5.3f MDM= %5.3e Mstars=  %5.3e %5.3e MISMs= %5.3e %5.3e MCGM= %5.3e MCGM_sub= %5.3e %5.3e %5.3e %5.3e %5.3e M_BH= %5.3e %5.3e\n"%(M200,Mstar,SFRcentral,M_DM,M_stars,M_stars_30kpc,M_ISM,M_ISM_30kpc,M_CGM,M_lt104,M_104_105,M_105_106,M_106_107,M_gt107,M_BH,max_M_BH))
fmasses.close()
...
# This calculates and makes hydrostatic equilibrium plots which went into figure 1 of my 2018 paper.  This is rather involved code in hse_spherical.py that I would skip for the time being.  
...
if dofigs:
    for k in xrange(3):
        ...
        if chem:
            ...
            lgrid = R200*1.5 #New as of 11-16-18, for Jon and Rob
            lgridz = lgrid*2 # Depth is twice the length of grid.

            print "Coords_Rotate_x_10percentiles= ", np.percentile(coords_gas_rot[:,0],0),np.percentile(coords_gas_rot[:,0],10),np.percentile(coords_gas_rot[:,0],20),np.percentile(coords_gas_rot[:,0],30),np.percentile(coords_gas_rot[:,0],40),np.percentile(coords_gas_rot[:,0],50),np.percentile(coords_gas_rot[:,0],60),np.percentile(coords_gas_rot[:,0],70),np.percentile(coords_gas_rot[:,0],80),np.percentile(coords_gas_rot[:,0],90),np.percentile(coords_gas_rot[:,0],100),len(coords_gas_rot[:,0]),boxsize_pMpc,gal_coords_pMpc

            print "Coords_Shift_x_10percentiles= ", np.percentile(coords_gas_shift[:,0],0),np.percentile(coords_gas_shift[:,0],10),np.percentile(coords_gas_shift[:,0],20),np.percentile(coords_gas_shift[:,0],30),np.percentile(coords_gas_shift[:,0],40),np.percentile(coords_gas_shift[:,0],50),np.percentile(coords_gas_shift[:,0],60),np.percentile(coords_gas_shift[:,0],70),np.percentile(coords_gas_shift[:,0],80),np.percentile(coords_gas_shift[:,0],90),np.percentile(coords_gas_shift[:,0],100),len(coords_gas_shift[:,0]),boxsize_pMpc,boxcenter_pMpc

            result_h1 = coldens.main(coords_gas_shift, hsmooth, mass_gas*hydrogen*h1, boxcenter_pMpc, lgrid, lgrid, lgridz, ngrid, ngrid, 58, boxsize_pMpc,fig_name='coldens.%s.h1.%s.l%3.1f.png'%(galidstr,dir,lgrid),Vmin=13.0, Vmax=21.0,ion='HI',theta=theta,phi=phi,psi=psi,redshift=redshift,extralabel='HI',haloinfostr=haloinfostr,docbar=1)
            write_radial_column('columnave_lin.%s.%s.%s.l%3.1f.dat'%(galidstr,"h1",dir,lgrid),10**result_h1,ngrid,lgrid,1,15.0,16.0,17.2)

            result_c4 = coldens.main(coords_gas_rot, hsmooth, mass_gas*hydrogen*c4, gal_coords_pMpc, lgrid, lgrid, lgridz, ngrid, ngrid, 58, boxsize_pMpc,fig_name='coldens.%s.c4.%s.l%3.1f.png'%(galidstr,dir,lgrid),Vmin=11.0, Vmax=15.0,ion='CIV',theta=theta,phi=phi,psi=psi,redshift=redshift,extralabel='CIV',haloinfostr=haloinfostr,docbar=1)
            write_radial_column('columnave_lin.%s.%s.%s.l%3.1f.dat'%(galidstr,"c4",dir,lgrid),10**result_c4,ngrid,lgrid,1,13.2,13.5,13.8)
            
            result_o6 = coldens.main(coords_gas_rot, hsmooth, mass_gas*hydrogen*o6, gal_coords_pMpc, lgrid, lgrid, lgridz, ngrid, ngrid, 58, boxsize_pMpc,fig_name='coldens.%s.o6.%s.l%3.1f.png'%(galidstr,dir,lgrid),Vmin=11.0, Vmax=15.0,ion='OVI',theta=theta,phi=phi,psi=psi,redshift=redshift,extralabel='OVI',haloinfostr=haloinfostr,docbar=1)
            write_radial_column('columnave_lin.%s.%s.%s.l%3.1f.dat'%(galidstr,"o6",dir,lgrid),10**result_o6,ngrid,lgrid,1,13.4,13.7,14.0)

            result_o7 = coldens.main(coords_gas_rot, hsmooth, mass_gas*hydrogen*o7, gal_coords_pMpc, lgrid, lgrid, lgridz, ngrid, ngrid, 58, boxsize_pMpc,fig_name='coldens.%s.o7.%s.l%3.1f.png'%(galidstr,dir,lgrid),Vmin=13.0, Vmax=17.0,ion='OVII',theta=theta,phi=phi,psi=psi,redshift=redshift,extralabel='OVII',haloinfostr=haloinfostr,docbar=1)
            write_radial_column('columnave_lin.%s.%s.%s.l%3.1f.dat'%(galidstr,"o7",dir,lgrid),10**result_o7,ngrid,lgrid,1,15.0,15.5,16.0)

            result_o8 = coldens.main(coords_gas_rot, hsmooth, mass_gas*hydrogen*o8, gal_coords_pMpc, lgrid, lgrid, lgridz, ngrid, ngrid, 58, boxsize_pMpc,fig_name='coldens.%s.o8.%s.l%3.1f.png'%(galidstr,dir,lgrid),Vmin=13.0, Vmax=17.0,ion='OVIII',theta=theta,phi=phi,psi=psi,redshift=redshift,extralabel='OVIII',haloinfostr=haloinfostr,docbar=1)
            write_radial_column('columnave_lin.%s.%s.%s.l%3.1f.dat'%(galidstr,"o8",dir,lgrid),10**result_o8,ngrid,lgrid,1,15.0,15.5,16.0)

            result_hyd = coldens.main(coords_gas_rot, hsmooth, mass_gas*hydrogen, gal_coords_pMpc, lgrid, lgrid, lgridz, ngrid, ngrid, 58, boxsize_pMpc,fig_name='coldens.%s.hyd.%s.l%3.1f.png'%(galidstr,dir,lgrid),Vmin=19.0, Vmax=21.0,ion='Hydrogen',theta=theta,phi=phi,psi=psi,redshift=redshift,extralabel='Hydrogen',haloinfostr=haloinfostr,docbar=1)
            write_radial_column('columnave_lin.%s.%s.%s.l%3.1f.dat'%(galidstr,"hyd",dir,lgrid),10**result_hyd,ngrid,lgrid,1,19.0,19.5,20.0)

            result_car = coldens.main(coords_gas_rot, hsmooth, mass_gas*hydrogen*car, gal_coords_pMpc, lgrid, lgrid, lgridz, ngrid, ngrid, 58, boxsize_pMpc,fig_name='coldens.%s.car.%s.l%3.1f.png'%(galidstr,dir,lgrid),Vmin=13.5, Vmax=17.5,ion='Carbon',theta=theta,phi=phi,psi=psi,redshift=redshift,extralabel='Carbon',haloinfostr=haloinfostr,docbar=1)
            write_radial_column('columnave_lin.%s.%s.%s.l%3.1f.dat'%(galidstr,"car",dir,lgrid),10**result_car,ngrid,lgrid,1,14.5,15.0,15.5)

            result_oxy = coldens.main(coords_gas_rot, hsmooth, mass_gas*hydrogen*oxy, gal_coords_pMpc, lgrid, lgrid, lgridz, ngrid, ngrid, 58, boxsize_pMpc,fig_name='coldens.%s.oxy.%s.l%3.1f.png'%(galidstr,dir,lgrid),Vmin=13.5, Vmax=17.5,ion='Oxygen',theta=theta,phi=phi,psi=psi,redshift=redshift,extralabel='Oxygen',haloinfostr=haloinfostr,docbar=1)
            write_radial_column('columnave_lin.%s.%s.%s.l%3.1f.dat'%(galidstr,"oxy",dir,lgrid),10**result_oxy,ngrid,lgrid,1,15.0,15.5,16.0)
