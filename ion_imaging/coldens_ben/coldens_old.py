...
def make_colourmap(fig_name, ResultW, Xmin, Xmax, Ymin, Ymax, Vmin, Vmax, ion, npix, redshift=None, extralabel=None, haloinfostr=None):
    ...
    fig = plt.figure(figsize = (npix/80.+1, npix/80.))
    ax = plt.subplot(111)
    ...
    if redshift != None:
        ax.text((Xmax-Xmin)*0.9+Xmin,(Ymax-Ymin)*0.9+Ymin,r'z = %4.2f'%(redshift),horizontalalignment='right', verticalalignment='top', fontsize=labelfontsize)
    if extralabel != None:
        ax.text((Xmax-Xmin)*0.1+Xmin,(Ymax-Ymin)*0.9+Ymin,r'%s'%(extralabel),horizontalalignment='left', verticalalignment='top', fontsize=labelfontsize)
    if haloinfostr != None:
        ax.text((Xmax-Xmin)*0.5+Xmin,(Ymax-Ymin)*0.975+Ymin,r'%s'%(haloinfostr),fontsize=extrastrfontsize, horizontalalignment='center', verticalalignment='top')


    # colour map
    if ion == "T" or ion == "temperature" or ion == "To6" or ion == "T(OVI)" or ion == "nH":
        ...
        if Vmin < 0.0 and Vmax > 0.0: # Difference map
            ...
            if Vmin < 0.0 and Vmax == 0.0: 
                img = ax.imshow(ResultW.T, extent=(Xmin,Xmax,Ymin,Ymax), cmap=cm.get_cmap("jet"), origin='lower', interpolation='nearest', vmin=Vmin, vmax=Vmax)
            else:
    ...
    if ion != None:
        if ion == "vx" or ion == "vy" or ion == "vz" or ion == "vdotroverr":
            ...
            if ion == "T":
                cbar.ax.set_ylabel(r'log T [K]')
            else:
                if ion == "nH":
                    cbar.ax.set_ylabel(r'log n$_{\mathrm{H}}$ [cm$^{-3}$]')
                else:
                    if ion == "Z":
                        cbar.ax.set_ylabel(r'log Z')
                    else:
                        if ion == "To6" or ion == "T(OVI)":
                            cbar.ax.set_ylabel(r'log T$_{\mathrm{OVI}}$ [K]')
                        else:
                            if(Vmin<0):
                                if(Vmax == 0):
                                    cbar.ax.set_ylabel(r'log %s'%(extralabel))
                                else:                                    
                                    cbar.ax.set_ylabel(r'$\Delta$log N [cm$^{-2}$]')                                #cbar.ax.set_ylabel(r'$\Delta$log N$_{\mathrm{%s}}$ [cm$^{-2}$]' % (extralabel))
                            else:
                                cbar.ax.set_ylabel(r'log N$_{\mathrm{%s}}$ [cm$^{-2}$]' % (extralabel))
    else:
        cbar.ax.set_ylabel(r'log N [cm$^{-2}$]')
...
def main(coords, l_smooth, mass_ion, centre, L_x, L_y, L_z, npix_x, npix_y, desngb, BoxSize, fig_name=None, Vmin=None, Vmax=None, ion=None, redshift=None, extralabel=None, haloinfostr=None, theta=0.0, phi=0.0, psi=0.0, npix=400):
    ...
    print "area = ", area, " c.u= ", c.u, " c.solar_mass= ", c.solar_mass
    ...
    if fig_name != None:
        ...
        if haloinfostr != None and redshift != None and extralabel != None:
            make_colourmap(fig_name, ResultW, Xmin, Xmax, Ymin, Ymax, Vmin, Vmax, ion, npix, redshift, extralabel, haloinfostr)
        else:
            if redshift != None and extralabel != None:
                make_colourmap(fig_name, ResultW, Xmin, Xmax, Ymin, Ymax, Vmin, Vmax, ion, npix, redshift, extralabel)
            else:
                if redshift != None:
                    make_colourmap(fig_name, ResultW, Xmin, Xmax, Ymin, Ymax, Vmin, Vmax, ion, npix, redshift, extralabel)
                else:
                    make_colourmap(fig_name, ResultW, Xmin, Xmax, Ymin, Ymax, Vmin, Vmax, ion, npix)
