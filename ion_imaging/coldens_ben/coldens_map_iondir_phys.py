...
if(ion=="h1"):
    ...
    colmax = 20
if(ion=="c2"):
    c2 = eagle.readArray("SNIP", sim, input_filename_base, "/PartType0/ChemicalAbundances/CarbonII",numThreads=1)
...
if(ion=="c4"):
    ...
    colmax = 15.5
if(ion=="n5"):
    n5 = eagle.readArray("SNIP", sim, input_filename_base, "/PartType0/ChemicalAbundances/NitrogenV",numThreads=1)
...
if(ion=="T"):
    ...
    colmax = 7.0
if(ion=="si2"):
    si2 = eagle.readArray("SNIP", sim, input_filename_base, "/PartType0/ChemicalAbundances/SiliconII",numThreads=1)
    ...
    colmax = 15
if(ion=="si3"):
    si3 = eagle.readArray("SNIP", sim, input_filename_base, "/PartType0/ChemicalAbundances/SiliconIII",numThreads=1)
    ...
    colmax = 15
if(ion=="si4"):
    si4 = eagle.readArray("SNIP", sim, input_filename_base, "/PartType0/ChemicalAbundances/SiliconIV",numThreads=1)
...
numerator_nH = nh[indexes]*denominator
numerator_T = temperature[indexes]*denominator
...
for i in xrange(ngrid):
    for j in xrange(ngrid):
        f.write('%3d %3d % 5.2f %5.1f % 5.2f %5.2f %6.1f\n'%(i,j,result_den[i,j],np.sqrt((ngrid/2.-0.5-j)**2+(ngrid/2.-0.5-i)**2)/ngrid*lgrid*1e+03,result_num_nH[i,j]-result_den[i,j],result_num_T[i,j]-result_den[i,j],10**(result_num_rad[i,j]-result_den[i,j])))
