...
for i in range(nslice):
    ...
    if(do_metals):
        ...
        result_Z = coldens.main(coords[indexes], hsmooth[indexes], ionmass[indexes]*metallicity[indexes], center, lgrid, lgrid, lgridz, ngrid, ngrid, 58, boxsize,fig_name='coldens_Z.%s.%s.%s.l%3.1f.%s.png'%(snapname,ionname,sliceinfostr,lgrid,enviro_label),Vmin=colmin, Vmax=colmax,ion=ionname,redshift=redshift,extralabel=ionname,haloinfostr=sliceinfostr,docbar=docbar)  

    sumslice = np.sum(result)

    sumslice = np.sum(10**(result[np.where(np.isfinite(result))]))
    ...
    for i in xrange(ngrid):
        for j in xrange(ngrid):
            if(result[i,j] >= coldens_limit):
                if(do_metals):
                    f.write('%3d %3d % 5.2f %5.3e\n'%(i,j,result[i,j],10**result_Z[i,j]/10**result[i,j]))
                else:
                    f.write('%3d %3d % 5.2f\n'%(i,j,result[i,j]))
