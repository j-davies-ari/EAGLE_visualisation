...
def make_colourmap(fig_name, ResultW, Xmin, Xmax, Ymin, Ymax, Vmin, Vmax, ion, npix, redshift=None, extralabel=None, haloinfostr=None, mhalo=None,docbar=None, topbottomlabel=None):
    ...
    # colour map
    if ion == "T" or ion == "temperature" or ion == "To6" or ion == "T(OVI)" or ion =="entropy":
        img = ax.imshow(ResultW.T, extent=(Xmin,Xmax,Ymin,Ymax), cmap=cm.get_cmap("gist_ncar"), origin='lower', interpolation='nearest', vmin=Vmin, vmax=Vmax)
    else:
    ...
    #cbar.ax.set_ylabel(r'log N$_{\mathrm{%s}}$(NEQ-Equ) [cm$^{-2}$]' % (ion))
        if ion != None:
            if ion == "vx" or ion == "vy" or ion == "vz" or ion == "vdotroverr":
                cbar.ax.set_xlabel(r'v [km/s]',labelpad=14,fontsize=extrastrfontsize)            
            else:
                if ((ion == "T") or (ion == "temperature")):
                    cbar.ax.set_xlabel(r'log$T$ [K]',labelpad=14,fontsize=extrastrfontsize)
                else:
                    if ion == "nH":
                        cbar.ax.set_xlabel(r'log$n_{\mathrm{H}}$ [cm$^{-3}$]',labelpad=14,fontsize=extrastrfontsize)
                    else:
                        if ion == "Z":
                            cbar.ax.set_xlabel(r'log$Z$',labelpad=14,fontsize=extrastrfontsize)
                        else:
                            if ion == "entropy":
                                cbar.ax.set_xlabel(r'log$S$ [keVcm$^{2}$]',labelpad=14,fontsize=extrastrfontsize-1)
                            else:
                                if ion == "To6" or ion == "T(OVI)":
                                    cbar.ax.set_xlabel(r'lg$T_{\mathrm{OVI}}$ [K]',labelpad=14,fontsize=extrastrfontsize)
                                else:
                                    if ion == "xOVI":
                                        cbar.ax.set_xlabel(r'lg$x_{\mathrm{OVI}}$',labelpad=14,fontsize=extrastrfontsize)
                                    else:
                                        if ion == "xOVII":
                                            cbar.ax.set_xlabel(r'lg$x_{\mathrm{OVII}}$',labelpad=14,fontsize=extrastrfontsize)
                                        else:
                                            if ion == "xOVIII":
                                                cbar.ax.set_xlabel(r'lg$x_{\mathrm{OVIII}}$',labelpad=14,fontsize=extrastrfontsize)
                                            else:
                                                if(Vmin<0):
                                                    if(Vmax <= 0):
                                                        cbar.ax.set_xlabel(r'lg%s'%(extralabel),labelpad=14,fontsize=extrastrfontsize)
                                                    else:
                                                        cbar.ax.set_xlabel(r'$\Delta$lg$N$ [cm$^{-2}$]',labelpad=14,fontsize=extrastrfontsize)                                #cbar.ax.set_xlabel(r'$\Delta$log N$_{\mathrm{%s}}$ [cm$^{-2}$]' % (extralabel))
                                                else:
                                                    cbar.ax.set_xlabel(r'lg$N_{\mathrm{%s}}$ [cm$^{-2}$]' % (extralabel),labelpad=14,fontsize=extrastrfontsize)
        else:
            cbar.ax.set_xlabel(r'%s'%(extralabel),labelpad=14,fontsize=extrastrfontsize)

    print 'labelfontsize =', labelfontsize
    if redshift != None:
        ax.text((Xmax-Xmin)*0.925+Xmin,(Ymax-Ymin)*0.95+Ymin,r'z = %4.2f'%(redshift),horizontalalignment='right', verticalalignment='top', fontsize=26)
    if mhalo != None:
        ax.text((Xmax-Xmin)*0.925+Xmin,(Ymax-Ymin)*0.95+Ymin,r'log M$_{200}$=%4.1f'%(mhalo),horizontalalignment='right', verticalalignment='top', fontsize=labelfontsize)
    if extralabel != None:
        ax.text((Xmax-Xmin)*0.075+Xmin,(Ymax-Ymin)*0.95+Ymin,r'%s'%(extralabel),horizontalalignment='left', verticalalignment='top', fontsize=26)
        #ax.text((Xmax-Xmin)*0.075+Xmin,(Ymax-Ymin)*0.05+Ymin,r'%s'%(extralabel),horizontalalignment='left', verticalalignment='bottom', fontsize=26)
    if haloinfostr != None:
        ax.text((Xmax-Xmin)*0.5+Xmin,(Ymax-Ymin)*0.075+Ymin,r'%s'%(haloinfostr),fontsize=extrastrfontsize+1, horizontalalignment='center', verticalalignment='top',zorder=100)
    if topbottomlabel != None: 
        ax.text((Xmax-Xmin)*0.925+Xmin,(Ymax-Ymin)*0.95+Ymin,r'%s'%(topbottomlabel),horizontalalignment='right', verticalalignment='top', fontsize=26)
        ax.text((Xmax-Xmin)*0.925+Xmin,(Ymax-Ymin)*0.05+Ymin,r'%s'%(topbottomlabel),horizontalalignment='right', verticalalignment='bottom', fontsize=26)
...
def main(coords, l_smooth, mass_ion, centre, L_x, L_y, L_z, npix_x, npix_y, desngb, BoxSize, fig_name=None, Vmin=None, Vmax=None, ion=None, redshift=None, mhalo=None, extralabel=None, haloinfostr=None, theta=0.0, phi=0.0, psi=0.0, docbar=None, emission=None):
    ...
    if fig_name != None:
        ...
        if mhalo != None:
            make_colourmap(fig_name, ResultW, Xmin, Xmax, Ymin, Ymax, Vmin, Vmax, ion, npix_x, extralabel=extralabel, mhalo=mhalo, docbar=docbar)
        else:
