...
lgrid = lgrid/1e+03
lgridz = lgrid*4 #Back to 4 here 3/25/18 #Now 2 times as of 12/30/14 # Four times longer than Lgrid in zdirection.
...
if(ion=="n5"):
    ...
    colmax = 15
if(ion=="o1"):
    o1 = eagle.readArray(sniptag, sim, input_filename_base, "/PartType0/ChemicalAbundances/OxygenI",numThreads=1)
    indexes = np.where(o1>1e-08)
    denominator = mass[indexes]*hydrogen[indexes]*o1[indexes]
    ionname = 'OI'
    colmin = 11
    colmax = 15
if(ion=="o2"):
    o2 = eagle.readArray(sniptag, sim, input_filename_base, "/PartType0/ChemicalAbundances/OxygenII",numThreads=1)
    indexes = np.where(o2>1e-08)
    denominator = mass[indexes]*hydrogen[indexes]*o2[indexes]
    ionname = 'OII'
    colmin = 11
    colmax = 15
if(ion=="o3"):
    o3 = eagle.readArray(sniptag, sim, input_filename_base, "/PartType0/ChemicalAbundances/OxygenIII",numThreads=1)
    indexes = np.where(o3>1e-08)
    denominator = mass[indexes]*hydrogen[indexes]*o3[indexes]
    ionname = 'OIII'
    colmin = 11
    colmax = 15
if(ion=="o4"):
    o4 = eagle.readArray(sniptag, sim, input_filename_base, "/PartType0/ChemicalAbundances/CarbonIV",numThreads=1)
    indexes = np.where(o4>1e-08)
    denominator = mass[indexes]*hydrogen[indexes]*o4[indexes]
    ionname = 'OIV'
    colmin = 11
    colmax = 15
if(ion=="o5"):
    o5 = eagle.readArray(sniptag, sim, input_filename_base, "/PartType0/ChemicalAbundances/OxygenV",numThreads=1)
    indexes = np.where(o5>1e-08)
    denominator = mass[indexes]*hydrogen[indexes]*o5[indexes]
    ionname = 'OV'
    colmin = 11
    colmax = 15
...
#make_colourmap(fig_name, ResultW, Xmin, Xmax, Ymin, Ymax, Vmin, Vmax, ion, npix, redshift)
    coldens.make_colourmap('coldens.%s.%s.%s.l%3.1f.png'%(snapname,ion,direction,lgrid), result_num-result_den, -1.0*lgrid/2.0, lgrid/2.0, -1.0*lgrid/2.0, lgrid/2.0, colmin, colmax, ion, ngrid, redshift=redshift, extralabel=ionname,kpc='True')
