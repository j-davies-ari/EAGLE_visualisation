...
radial_coords = np.abs(coords - gal_coords)
radial_coords = np.where(radial_coords > 0.5*boxsize*aex/hubble_param, boxsize*aex/hubble_param - radial_coords, radial_coords)
radial_dist = np.sqrt(radial_coords[:,0]**2+radial_coords[:,1]**2+radial_coords[:,2]**2)*1e+03
print "radial_dist= ", radial_dist
...
if(ion=="oxygen"):
    ...
    colmax = 18
if(ion=="h1"):
    if(snip=='1'): h1 = eagle.readArray("SNIP", sim, input_filename_base, "/PartType0/ChemicalAbundances/HydrogenI",numThreads=1)
    ...
    colmax = 20
if(ion=="c2"):
    if(snip=='1'): c2 = eagle.readArray("SNIP", sim, input_filename_base, "/PartType0/ChemicalAbundances/CarbonII",numThreads=1)
    ...
    colmax = 15
if(ion=="c4"):
    if(snip=='1'): c4 = eagle.readArray("SNIP", sim, input_filename_base, "/PartType0/ChemicalAbundances/CarbonIV",numThreads=1)
    ...
    colmax = 15.5
if(ion=="n5"):
    if(snip=='1'): n5 = eagle.readArray("SNIP", sim, input_filename_base, "/PartType0/ChemicalAbundances/NitrogenV",numThreads=1)
...
if(ion=="o6" or ion=="To6"):
    if(snip=='1'): o6 = eagle.readArray("SNIP", sim, input_filename_base, "/PartType0/ChemicalAbundances/OxygenVI",numThreads=1)
    ...
    colmax = 15.5
if(ion=="o7"):
    if(snip=='1'): o7 = eagle.readArray("SNIP", sim, input_filename_base, "/PartType0/ChemicalAbundances/OxygenVII",numThreads=1)
    ...
    colmax = 16.5
if(ion=="o8"):
    if(snip=='1'): o8 = eagle.readArray("SNIP", sim, input_filename_base, "/PartType0/ChemicalAbundances/OxygenVIII",numThreads=1)
    ...
    colmax = 16.5
if(ion=="mg2"):
    if(snip=='1'): mg2 = eagle.readArray("SNIP", sim, input_filename_base, "/PartType0/ChemicalAbundances/MagnesiumII",numThreads=1)
...
if(ion=="T"):
    indexes = np.where(mass>0.0)
    temperature = eagle.readArray("SNIP", sim, input_filename_base, "/PartType0/Temperature",numThreads=1)
    ...
    colmax = 7.0
if(ion=="si2"):
    if(snip=='1'): si2 = eagle.readArray("SNIP", sim, input_filename_base, "/PartType0/ChemicalAbundances/SiliconII",numThreads=1)
    ...
    colmax = 15
if(ion=="si3"):
    if(snip=='1'): si3 = eagle.readArray("SNIP", sim, input_filename_base, "/PartType0/ChemicalAbundances/SiliconIII",numThreads=1)
    ...
    colmax = 15
if(ion=="si4"):
    if(snip=='1'): si4 = eagle.readArray("SNIP", sim, input_filename_base, "/PartType0/ChemicalAbundances/SiliconIV",numThreads=1)
...
if(ion=="temperature"):
    indexes = np.where(mass>0.0)
    temperature = eagle.readArray("SNIP", sim, input_filename_base, "/PartType0/Temperature",numThreads=1)
    entropy = eagle.readArray("SNIP", sim, input_filename_base, "/PartType0/Entropy",numThreads=1)
    internalenergy = eagle.readArray("SNIP", sim, input_filename_base, "/PartType0/InternalEnergy",numThreads=1)
    nh = eagle.readArray("SNIP", sim, input_filename_base, "/PartType0/Density",numThreads=1)
    numerator = mass[indexes]*temperature[indexes]/nh[indexes]**(2/3.)*internalenergy[indexes]/(entropy[indexes]*3/2.) #NEVER MIND*aex**2 # Added for corrupted temperaturs 6-7-16  
    denominator = mass[indexes]
    ionname = 'T'
    colmin = 4.0
    colmax = 7.0
if(ion=="nH"):
    indexes = np.where(mass>0.0)
    nh = eagle.readArray("SNIP", sim, input_filename_base, "/PartType0/Density",numThreads=1)*unitDensity_in_cgs/proton_mass_cgs
    numerator = mass[indexes]*nh[indexes]
    denominator = mass[indexes]
    ionname = 'n$_{\mathrm{H}}$'
    colmin = -6.0
    colmax = -1.0
if(ion=="Z"):
    metallicity = eagle.readArray("SNIP", sim, input_filename_base, "/PartType0/Metallicity",numThreads=1)
...
weighted_density = aex**3. * (3/2.)**(3/2.) * (internalenergy/entropy)**(3/2.) * hubble_param**2.

nh_density = density*unitDensity_in_cgs/proton_mass_cgs*hydrogen
nh_weighteddensity = weighted_density*unitDensity_in_cgs/proton_mass_cgs*hydrogen
#nh = eagle.readArray(sniptag, sim, input_filename_base, "/PartType0/Density",numThreads=1)*unitDensity_in_cgs/proton_mass_cgs    
#nh = nh*hydrogen


pressure = density*unitDensity_in_cgs/proton_mass_cgs*temperature
pressure_WD = weighted_density*unitDensity_in_cgs/proton_mass_cgs*temperature

numerator_nH = nh_density[indexes]*denominator
numerator_T = temperature[indexes]*denominator  ###*aex**2 Taking out as of 5/5/17 because of fixed snapshots # Need this for noneq, but not ioneq. 6-7-15
numerator_rad = radial_dist[indexes]*denominator
numerator_nHWD = nh_weighteddensity[indexes]*denominator
numerator_P = pressure[indexes]*denominator
numerator_PWD = pressure_WD[indexes]*denominator
...
#if(ion=="temperature" or ion=="T" or ion=="nH" or ion=="Z" or ion=="To6"):
#    result_num = coldens.main(coords[indexes], hsmooth[indexes], numerator, center, lgrid, lgrid, lgridz, ngrid, ngrid, 58, boxsize,fig_name='num.png',Vmin=colmin, Vmax=colmax,ion=ion,npix=ngrid,theta=thetaangle,phi=phiangle,redshift=redshift,extralabel=ionname)
...
for i in xrange(ngrid):
    for j in xrange(ngrid):
        f.write('%3d %3d % 5.2f %5.1f % 5.2f %5.2f %5.2f %6.1f % 5.2f %5.2f\n'%(i,j,result_den[i,j],np.sqrt((ngrid/2.-0.5-j)**2+(ngrid/2.-0.5-i)**2)/ngrid*lgrid*1e+03,result_num_nH[i,j]-result_den[i,j],result_num_T[i,j]-result_den[i,j],result_num_P[i,j]-result_den[i,j],10**(result_num_rad[i,j]-result_den[i,j]),result_num_nHWD[i,j]-result_den[i,j],result_num_PWD[i,j]-result_den[i,j]))
